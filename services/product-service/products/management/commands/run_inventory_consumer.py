"""Kafka consumer that reserves/releases inventory from order events.

Run as a long-lived worker process (separate from the web server):

    python manage.py run_inventory_consumer

It subscribes to the order topics, dedupes each message by ``event_id`` (via the
``ProcessedEvent`` table) so redelivery is safe, and reserves or releases stock
accordingly. Offsets are committed only after a message is handled, so a crash
mid-processing re-delivers rather than loses the event.
"""
...
from products import consumer_metrics, events, inventory
...
class Command(BaseCommand):
    help = "Consume order events and reserve/release inventory."
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Inventory consumer listening on {events.CONSUMED_TOPICS}")
        )
    ...
    def _route(self, topic, order_id, data):
        if topic == events.TOPIC_ORDER_CREATED:
            inventory.reserve_for_order(
                order_id=order_id,
        ...
        elif topic == events.TOPIC_ORDER_CANCELLED:
            inventory.release_for_order(order_id)
        else:
